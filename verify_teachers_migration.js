...
async function verify() {
  ...
  console.log('\n=== 2. VERIFYING 4 TEACHERS TABLES SPECIFICALLY ===');
  const teacherTables = [
    'specializations',
    'teachers',
    'teacher_subjects',
  ...
  const resFKs = await client.query(`
    ...
    WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_schema = 'public'
      AND tc.table_name IN ('specializations', 'teachers', 'teacher_subjects', 'teacher_assignments')
  ...
  console.log('\n=== 4. VERIFYING PARTIAL UNIQUE INDEXES (Teachers Domain) ===');
  const resIndexes = await client.query(`
    SELECT tablename, indexname, indexdef
    FROM pg_indexes
    WHERE schemaname = 'public' 
      AND tablename IN ('specializations', 'teachers', 'teacher_assignments')
      AND indexname LIKE 'uq_%'
    ORDER BY tablename, indexname;
  `);
  resIndexes.rows.forEach(r => console.log(`  - ${r.tablename} -> ${r.indexname}: ${r.indexdef}`));

  console.log('\n=== 5. VERIFYING BLIND INDEXES & SEARCH INDEXES (Teachers) ===');
  const resSearchIdx = await client.query(`
    SELECT tablename, indexname, indexdef
    FROM pg_indexes
    WHERE schemaname = 'public' 
      AND tablename IN ('teachers', 'teacher_subjects', 'teacher_assignments')
      AND (indexname LIKE '%_hash_idx' OR indexname LIKE '%_key' OR indexname LIKE '%_idx')
    ORDER BY tablename, indexname;
  `);
  resSearchIdx.rows.forEach(r => console.log(`  - ${r.tablename} -> ${r.indexname}`));

  console.log('\n=== 6. VERIFYING ABSENCE OF FORBIDDEN TABLES (Guardians, Import, Promotion) ===');

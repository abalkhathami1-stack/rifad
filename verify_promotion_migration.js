...
async function verify() {
  ...
  const resFKs = await client.query(`
    ...
    WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_schema = 'public'
      AND tc.table_name IN ('promotion_batches', 'promotion_batch_items')
  ...
  console.log('\n=== 4. VERIFYING ENUMS (PromotionBatchStatus & PromotionAction) ===');
  const resEnums = await client.query(`
    ...
    WHERE n.nspname = 'public' AND t.typname IN ('PromotionBatchStatus', 'PromotionAction')
    GROUP BY t.typname
    ORDER BY t.typname;
  `);
  resEnums.rows.forEach(r => console.log(`  - ${r.enum_name}: [${r.values}]`));

  console.log('\n=== 5. VERIFYING INDEXES (Promotion Module) ===');
  const resIdx = await client.query(`
    SELECT tablename, indexname, indexdef
    FROM pg_indexes
    WHERE schemaname = 'public' 
      AND tablename IN ('promotion_batches', 'promotion_batch_items')
    ORDER BY tablename, indexname;
  `);
  resIdx.rows.forEach(r => console.log(`  - ${r.tablename} -> ${r.indexname}: ${r.indexdef}`));

  console.log('\n=== 6. VERIFYING ABSENCE OF FORBIDDEN TABLES ===');
  const forbiddenTables = [
    'guardians', 'parents',
    'import_batches', 'import_records', 'import_errors',
    'attendance', 'attendances', 'student_attendance',
    'assessments', 'student_grades', 'gradebook', 'exams'
  ];

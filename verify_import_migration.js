...
async function verify() {
  ...
  console.log('\n=== 2. VERIFYING 3 IMPORT TABLES SPECIFICALLY ===');
  const importTables = [
    'import_batches',
    'import_records',
    'import_errors'
  ...
  const resFKs = await client.query(`
    ...
    WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_schema = 'public'
      AND tc.table_name IN ('import_batches', 'import_records', 'import_errors')
  ...
  console.log('\n=== 4. VERIFYING ENUMS (ImportBatchStatus & ImportRecordStatus) ===');
  const resEnums = await client.query(`
    ...
    WHERE n.nspname = 'public' AND t.typname IN ('ImportBatchStatus', 'ImportRecordStatus')
    GROUP BY t.typname
    ORDER BY t.typname;
  `);
  resEnums.rows.forEach(r => console.log(`  - ${r.enum_name}: [${r.values}]`));

  console.log('\n=== 5. VERIFYING INDEXES (Import Engine) ===');
  const resIdx = await client.query(`
    SELECT tablename, indexname, indexdef
    FROM pg_indexes
    WHERE schemaname = 'public' 
      AND tablename IN ('import_batches', 'import_records', 'import_errors')
